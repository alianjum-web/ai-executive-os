...
| Area | Status | Notes |
|------|--------|-------|
| Frontend scaffold | Done | Next.js 16 (App Router), React 19, Tailwind CSS 4, TypeScript |
| Default home page | Placeholder | `src/app/page.tsx` — replace with dashboard shell |
| Backend (`/backend`) | Not started | FastAPI + LangGraph planned |
| PostgreSQL + pgvector | Not started | — |
| Redux Toolkit | **Planned** | Team decision: use Redux, not Zustand (see §6) |
| Shadcn/UI | Not started | Install when building atoms/molecules |
| Feature flags | Not started | Add `src/config/features.config.ts` in Sprint 1 |
| Auth (NextAuth / Supabase) | Not started | — |
| Knowledge Agent (RAG) | Not started | — |
| Project Agent (routing) | Not started | Sprint 3+ |

### 1.2 Active Sprint

**Sprint 0 (bootstrap)** — In progress

- [x] Initialize Next.js frontend repo
- [ ] Adopt folder structure from §4.1
- [ ] Install Redux Toolkit + configure store provider
- [ ] Implement hook abstraction layer (`useSidebar`, `useChat`, etc.)
- [ ] Add `features.config.ts` and `useFeatureFlag`
